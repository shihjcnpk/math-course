# 七八年级数学代表性课程：第24讲 全等三角形判定：SSS、SAS、ASA、AAS、HL

> 代表内容：全等三角形  
> 原始文件：`src/data/lectures/lecture-24.ts`  
> 教材版本：人教版新课标六三制
...
1. **所属知识主线：** 几何推理主线。
2. **对应教材章节：** 第14章 全等三角形。
3. **本节核心问题：** 怎样根据已知条件选择正确的全等判定？
4. **注意力缺陷支持设计：** 以“只判断一组条件能否判全等。”降低启动门槛；用“检查角是否夹在两条已知边之间。”作为做题前动作，并采用3+8+10+3+10+5+2分钟分段。
5. **错题闭环设计：** 保留原错误过程，选择主要/次要错因，回答错题四问，再用同核心、不同表面的变式题验证迁移。
6. **间隔复习设计：** D0原题重做、D1同类基础、D3变式、D7混合、D14综合；本讲变式任务为“把一个SAS条件中的夹角换成非夹角，判断为何失效。”。
