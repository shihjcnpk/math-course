...
function supportingSceneFor(lectureId: number, problem: string, context: 'example' | 'exercise' | 'oral' | 'answer'): Scene | null {
  ...
  if (lectureId === 1) {
    if (/数→式→方程→函数|代数推理链/.test(problem)) return { title: '代数知识递进链', description: '从数的运算出发，经由字母表示和方程求解，最终发展为研究变量关系的函数。', extra: <AlgebraKnowledgeChain /> }
    if (/s随t|s=vt/.test(problem)) return { title: '路程—时间正比例关系', description: '横轴是时间t，纵轴是路程s；速度v固定时，图像是经过原点的直线。', extra: <DistanceTimeFunction /> }
  ...
  if (lectureId === 47) {
    if (isNumberLineProblem(problem)) return numberLineScene(problem)
    if (problem.includes('AB=AC') && problem.includes('AE=AF') && problem.includes('D是BC的中点')) return { title: '等腰三角形中的对称全等', description: 'AB=AC、AE=AF推出BE=CF；再结合BD=CD和两底角相等，用SAS证明△BDE≅△CDF。', extra: <IsoscelesInnerPoints /> }
    if (/一次函数|函数图像|y=|直线/.test(problem)) return functionAnswerScene(problem)
    if (/统计图|平均数|中位数|方差/.test(problem)) return statisticsAnswerScene(problem)
    return { title: '题目条件结构图', description: '逐项把题干条件标到图上，再按知识链检查结论。', extra: <GenericGeometry /> }
  }
...
function FoldCToA() { return <g><rect x="150" y="50" width="240" height="200" fill="#eff6ff" stroke="#2563eb" strokeWidth="3.5" /><line x1="150" y1="250" x2="390" y2="50" stroke="#64748b" strokeWidth="3" strokeDasharray="8 7" /><line x1="150" y1="94" x2="390" y2="206" stroke="#e11d48" strokeWidth="4" /><path d="M365 65 Q300 125 175 230" fill="none" stroke="#7c3aed" strokeWidth="4" strokeDasharray="8 6" /><DiagramPoint at={[150,250]} label="A" dx={-14} dy={20} /><DiagramPoint at={[390,250]} label="B" dx={14} dy={20} /><DiagramPoint at={[390,50]} label="C" dx={14} dy={-8} /><DiagramPoint at={[150,50]} label="D" dx={-14} dy={-8} /><DiagramPoint at={[150,94]} label="E" dx={-15} dy={0} tone="accent" /><DiagramPoint at={[390,206]} label="F" dx={15} dy={0} tone="accent" /><DiagramLabel at={[270,165]} text="折痕 EF" tone="accent" /><DiagramLabel at={[320,95]} text="C→A" tone="proof" size={16} /></g> }

function MidpointParallelExtension() { return <g><line x1="115" y1="70" x2="260" y2="70" stroke="#2563eb" strokeWidth="4" /><line x1="340" y1="240" x2="500" y2="240" stroke="#2563eb" strokeWidth="4" /><line x1="260" y1="70" x2="340" y2="240" stroke="#334155" strokeWidth="3.5" /><line x1="115" y1="70" x2="500" y2="240" stroke="#e11d48" strokeWidth="3.5" /><DiagramPoint at={[115,70]} label="A" /><DiagramPoint at={[260,70]} label="B" /><DiagramPoint at={[340,240]} label="C" dy={22} /><DiagramPoint at={[500,240]} label="F" dy={22} /><DiagramPoint at={[300,155]} label="E" dx={14} dy={0} tone="accent" /><DiagramLabel at={[185,54]} text="AB" tone="primary" size={15} /><DiagramLabel at={[420,224]} text="CF" tone="primary" size={15} /><DiagramLabel at={[470,55]} text="AB∥CF" tone="proof" size={16} /></g> }

function IsoscelesInnerPoints() { return <g><polygon points="300,35 85,250 515,250" fill="#eff6ff" stroke="#2563eb" strokeWidth="3.5" /><line x1="300" y1="250" x2="190" y2="145" stroke="#e11d48" strokeWidth="3.5" /><line x1="300" y1="250" x2="410" y2="145" stroke="#e11d48" strokeWidth="3.5" /><Tick a={[85,250]} b={[300,250]} tone="primary" /><Tick a={[300,250]} b={[515,250]} tone="primary" /><Tick a={[300,35]} b={[190,145]} count={2} tone="proof" /><Tick a={[300,35]} b={[410,145]} count={2} tone="proof" /><DiagramPoint at={[300,35]} label="A" /><DiagramPoint at={[85,250]} label="B" dx={-12} dy={20} /><DiagramPoint at={[515,250]} label="C" dx={12} dy={20} /><DiagramPoint at={[300,250]} label="D" dy={22} /><DiagramPoint at={[190,145]} label="E" dx={-14} dy={0} /><DiagramPoint at={[410,145]} label="F" dx={14} dy={0} /><DiagramLabel at={[240,195]} text="DE" tone="accent" /><DiagramLabel at={[360,195]} text="DF" tone="accent" /><DiagramLabel at={[300,278]} text="BD=DC" tone="primary" size={15} /><DiagramLabel at={[300,75]} text="AE=AF" tone="proof" size={15} /></g> }
...
function AlgebraKnowledgeChain() { return <g>{[['数',95,'有理数运算'],['式',235,'整式与分式'],['方程',375,'求未知量'],['函数',515,'研究变量关系']].map(([label,x,note],index)=><g key={String(label)}><rect x={Number(x)-55} y="100" width="110" height="70" rx="12" fill={index % 2 ? '#ede9fe' : '#dbeafe'} stroke={index % 2 ? '#7c3aed' : '#2563eb'} strokeWidth="3" /><DiagramLabel at={[Number(x),128]} text={String(label)} tone={index % 2 ? 'proof' : 'primary'} size={20} /><DiagramLabel at={[Number(x),153]} text={String(note)} tone="muted" size={12} />{index<3&&<><line x1={Number(x)+58} y1="135" x2={Number(x)+78} y2="135" stroke="#e11d48" strokeWidth="3" /><path d={`M${Number(x)+78} 135 l-10 -6 v12z`} fill="#e11d48" /></>}</g>)}</g> }

function DistanceTimeFunction() { return <g><line x1="95" y1="265" x2="535" y2="265" stroke="#334155" strokeWidth="4" /><line x1="95" y1="265" x2="95" y2="35" stroke="#334155" strokeWidth="4" /><path d="M535 265 l-12 -7 v14z M95 35 l-7 12 h14z" fill="#334155" /><line x1="95" y1="265" x2="480" y2="65" stroke="#2563eb" strokeWidth="4" /><DiagramPoint at={[95,265]} label="O" dx={-16} dy={20} /><DiagramLabel at={[530,290]} text="时间 t" tone="base" size={16} /><DiagramLabel at={[65,45]} text="路程 s" tone="base" size={16} /><DiagramLabel at={[390,95]} text="s=vt" tone="primary" size={18} /><DiagramLabel at={[330,220]} text="斜率=速度v" tone="proof" size={15} /></g> }
